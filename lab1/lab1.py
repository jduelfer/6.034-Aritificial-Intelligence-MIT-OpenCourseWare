...
# family_rules.
sameidentity = IF( OR('male (?x)',
                          'female (?x)'), THEN('sameidentity (?x) (?x)'))
...
grandchild_rule = IF( AND('parent (?y) (?x)',
                          'parent (?z) (?y)'),
                        THEN('grandchild (?x) (?z)'))
sibling = IF( OR('brother (?x) (?y)',
                          'sister (?x) (?y)',
                          NOT('sameidentity (?x) (?y)')),
                        THEN('sibling (?x) (?y)'))
...
WHAT_I_FOUND_INTERESTING = 'Back chaining is really cool and helps me with recursion.'
WHAT_I_FOUND_BORING = 'Foward chaining seems really manual and dependent on a ton of rules.'
